...
error_reporting(0);
	...
	class Home extends Controllers{
        ...
        public function usuario(){
            ...
            if($token['token']){
                ...
                if($rol == "estudiante"){
                    ...
                    if($datos_usuario){
                        $_SESSION['userid'] = $datos_usuario['userid'];
        ...
        public function encuesta(){
            ...
            $modalidad = base64_decode($datos['modalidad']);
            ...
            if($idEncuesta == 12 || $idEncuesta == 15){
            ...
            if($idEncuesta == 25 || $idEncuesta == 26 || $idEncuesta == 27 || $idEncuesta == 14){
            ...
            }else if($idEncuesta == 13 || $idEncuesta == 16){
        ...
        public function modeloEducativo(){
         ...
         public function recRespuestasHeteroevaluacionAlumno(){
            $valor_r = $_GET['res'];
            $valor_d = $_GET['dat'];
            $valor_modalidad = $_GET['modalidad'];
            $valor_idencuesta = $_GET['id_encuesta'];
            $valor_res = json_decode($valor_r,JSON_UNESCAPED_UNICODE);
            $valor_dat = json_decode($valor_d,JSON_UNESCAPED_UNICODE);
            $modalidad = base64_decode($valor_modalidad);
            $idEncuesta = base64_decode($valor_idencuesta);
            $valor['res'] = $valor_res;
            $valor['dat'] = $valor_dat;
            $valor['modalidad'] = $modalidad;
            $valor['id_encuesta'] = $idEncuesta;
        ...
        public function getEncuestaPorId(int $id){
            $idEnc = $id;
            $request = $this->model->consultarEncuestaActiva($idEnc);
            echo json_encode($request, JSON_UNESCAPED_UNICODE);
            die();
        }
	}
