...
class HomeModel extends Mysql
{
    ...
    public function consultarDatosMateria($data){
        ...
        if(empty($datos['req_mat']))
        {
            ...
            $req_insDocente =$this->insert($sqlDocente, array($request[0]['id_user'],$request[0]['teacher_username'],$request[0]['teacher_firstname'],$request[0]['teacher_lastname']));
            ...
            $this->insert($sqlNuevaMat, array($id_course, $idDocente, $plataforma, $request[0]['fullname_course'], $request[0]['category_course']));
        }
		return $datos;
    }
    ...
    public function guardadResultadoHeteroevaluacionAlumnoBD($data){
        ...
        foreach ($data['res'] as $valores) {
            ...
            $this->insert($sql,array(11,$id_pregunta, $id_materia_bd, $id_docente_bd, $id_usuario_bd, $resultados[$respuesta],1,100));
        }
        return $data;
    }
    ...
    function selectEstatusMaterias($idEncuesta,$username,$plataforma,$materias)
    {
        ...
        foreach ($materias as $key => $materia) {
            $nombreUsuario = $materia['student_username'];
